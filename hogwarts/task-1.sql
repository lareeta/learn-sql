-- 1.Выведите имя, фамилию, patronus всех персонажей, у которых есть patronus и он известен
SELECT fname, lname, patronus 
FROM characters
WHERE patronus <> 'Unknown' AND patronus IS NOT NULL;

-- 2.Выведите фамилию персонажей, у которых последняя буква в фамилии "е"
SELECT lname 
FROM characters
WHERE lname
LIKE '%e';
...
-- 7.Выведите всю информацию о персонажах, у которых faculty = Gryffindor и чей возраст больше 30
...
-- 12.Выведите фамилию всех персонажей, которые содержат в ней букву "а"
SELECT lname
FROM characters
WHERE lname LIKE '%a%';

-- 13.Используйте псевдоним для того, чтобы временно заменить название столбца fname и Half-Blood Prince для реального принца-полукровки
